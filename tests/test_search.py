...
def test_paginated_search_limits(multi_page_search_client):
    ...
    results = search.fetch_all()
    assert len(results) == len(get_canned_search_results_multi_page_page1()["features"]) + len(
        get_canned_search_results_multi_page_page2()["features"]
    )
...
def test_search_result_to_feature_collection(multi_page_search_client):
    ...
    page_ret = [
        *get_canned_search_results_multi_page_page1()["features"],
        *get_canned_search_results_multi_page_page2()["features"],
    ]
...
def test_paginated_search_multi_page(single_page_search_client):
    search = StacSearch(single_page_search_client._sesh, limit=1)
    results = search.fetch_all()
    assert len(results) == 1
    assert results[0] == get_canned_search_results_single_page()["features"][0]
...
def test_threaded_paginated_search_multi_page(single_page_search_client):
    search = StacSearch(single_page_search_client._sesh, limit=1, threaded=True)
    results = search.fetch_all()
    assert len(results) == 1
    assert results[0] == get_canned_search_results_single_page()["features"][0]
